...
func TestConfig(t *testing.T) {
		...
		Convey("Test GetMTLSConfig()", func() {
			...
			authConfig = &config.AuthConfig{
				MTLS: &config.MTLSConfig{
					IdentitySources: []string{"CommonName", "URL"},
					URISANPattern:   "spiffe://example.org/workload/(.*)",
				},
			}
			mtlsConfig := authConfig.GetMTLSConfig()
			So(mtlsConfig, ShouldNotBeNil)
			So(len(mtlsConfig.IdentitySources), ShouldEqual, 2)
			So(mtlsConfig.IdentitySources[0], ShouldEqual, "CommonName")
			So(mtlsConfig.IdentitySources[1], ShouldEqual, "URL")
	...
	Convey("Test Config getter methods", t, func() {
		Convey("Test CopyAuthConfig()", func() {
			...
			Convey("Test that returned AuthConfig is isolated from config mutations", func() {
				...
				cfg := &config.Config{
					HTTP: config.HTTPConfig{
						Auth: &config.AuthConfig{
							...
							MTLS: &config.MTLSConfig{
								IdentitySources: []string{"CommonName"},
								URISANPattern:   "spiffe://example.org/workload/(.*)",
							},
						},
					},
				...
				So(authConfig.GetMTLSConfig().IdentitySources[0], ShouldEqual, "CommonName")
				...
				authConfig.MTLS.IdentitySources[0] = "URL"
				...
				So(cfg.HTTP.Auth.MTLS.IdentitySources[0], ShouldEqual, "CommonName")
	...
	Convey("Test UpdateReloadableConfig MTLS config updates", t, func() {
		Convey("Test MTLS config is updated in UpdateReloadableConfig", func() {
			...
			newConfig := &config.Config{
				HTTP: config.HTTPConfig{
					Auth: &config.AuthConfig{
						MTLS: &config.MTLSConfig{
							IdentitySources: []string{"URL", "CommonName"},
			...
			So(len(cfg.HTTP.Auth.MTLS.IdentitySources), ShouldEqual, 2)
			So(cfg.HTTP.Auth.MTLS.IdentitySources[0], ShouldEqual, "URL")
			So(cfg.HTTP.Auth.MTLS.IdentitySources[1], ShouldEqual, "CommonName")
		...
		Convey("Test MTLS config is created when going from nil to non-nil", func() {
			...
			So(cfg.HTTP.Auth.MTLS, ShouldNotBeNil)
			So(len(cfg.HTTP.Auth.MTLS.IdentitySources), ShouldEqual, 1)
			So(cfg.HTTP.Auth.MTLS.IdentitySources[0], ShouldEqual, "URL")
			So(cfg.HTTP.Auth.MTLS.URISANPattern, ShouldEqual, "spiffe://new.example.org/workload/(.*)")
		})
	})
