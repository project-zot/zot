...
func TestConfig(t *testing.T) {
	...
	Convey("Test AccessControlConfig methods", t, func() {
		...
		Convey("Test SetRepositories()", func() {
			...
			accessControlConfig.SetRepositories(repositories)
		...
		Convey("Test SetAdminPolicy()", func() {
			...
			accessControlConfig.SetAdminPolicy(adminPolicy)
		...
		Convey("Test SetMetrics()", func() {
			...
			accessControlConfig.SetMetrics(metrics)
		...
		Convey("Test SetGroups()", func() {
			...
			accessControlConfig.SetGroups(groups)
	...
	Convey("Test Config getter methods", t, func() {
		Convey("Test GetAuthConfig()", func() {
			...
			Convey("Test that returned AuthConfig is isolated from config mutations", func() {
				...
				cfg.HTTP.Auth.FailDelay = 10
				cfg.HTTP.Auth.HTPasswd.Path = "/etc/new-htpasswd"
				cfg.HTTP.Auth.APIKey = true

				// Verify that the returned AuthConfig is not affected by the mutations
				...
				So(newAuthConfig.GetFailDelay(), ShouldEqual, 10)       // Should be updated
				So(newAuthConfig.IsHtpasswdAuthEnabled(), ShouldBeTrue) // Should be updated
				So(newAuthConfig.IsAPIKeyEnabled(), ShouldBeTrue)       // Should be updated
			})
		...
		Convey("Test GetHTTPConfig()", func() {
			Convey("Test with non-nil HTTP", func() {
				cfg := &config.Config{
					HTTP: config.HTTPConfig{
						Address: "127.0.0.1",
						Port:    "8080",
				...
				So(httpConfig, ShouldNotBeNil)
				So(httpConfig.Address, ShouldEqual, "127.0.0.1")
				So(httpConfig.Port, ShouldEqual, "8080")
			})

			Convey("Test with nil HTTP", func() {
				cfg := &config.Config{
					HTTP: config.HTTPConfig{},
				}
				httpConfig := cfg.GetHTTPConfig()
				So(httpConfig, ShouldNotBeNil) // HTTPConfig is a struct, not a pointer, so it's never nil
				So(httpConfig.Address, ShouldEqual, "")
				So(httpConfig.Port, ShouldEqual, "")
			})
		})
	...
	Convey("Test nil receiver coverage for all methods", t, func() {
		...
		Convey("Test AccessControlConfig methods with nil receiver", func() {
			...
			So(func() { accessControlConfig.SetRepositories(config.Repositories{}) }, ShouldNotPanic)
			So(func() { accessControlConfig.SetAdminPolicy(config.Policy{}) }, ShouldNotPanic)
			So(func() { accessControlConfig.SetMetrics(config.Metrics{}) }, ShouldNotPanic)
			So(func() { accessControlConfig.SetGroups(config.Groups{}) }, ShouldNotPanic)
		...
		Convey("Test Config methods with nil receiver", func() {
			...
			So(cfg.GetHTTPConfig(), ShouldResemble, config.HTTPConfig{})
	...
	Convey("Test AccessControlConfig mutation through GetAccessControlConfig()", t, func() {
		Convey("Test that mutations to retrieved AccessControlConfig are reflected in original config", func() {
			...
			// Retrieve the AccessControlConfig
			accessControlConfig := cfg.GetAccessControlConfig()
			So(accessControlConfig, ShouldNotBeNil)

			// Mutate the retrieved AccessControlConfig
			accessControlConfig.SetAdminPolicy(config.Policy{
			...
			accessControlConfig.SetRepositories(newRepositories)

			// Verify that the changes are reflected in the original config
			updatedAccessControlConfig := cfg.GetAccessControlConfig()
			So(updatedAccessControlConfig, ShouldNotBeNil)

			// Check that admin policy was updated
			adminPolicy := updatedAccessControlConfig.GetAdminPolicy()
			So(adminPolicy.Actions, ShouldResemble, []string{"read", "write", "delete"})
			So(adminPolicy.Users, ShouldResemble, []string{"admin", "superadmin"})

			// Check that repositories were updated
			repositories := updatedAccessControlConfig.GetRepositories()
			So(len(repositories), ShouldEqual, 2)
			So(repositories["repo1"], ShouldNotBeNil)
			So(repositories["repo2"], ShouldNotBeNil)
			So(repositories["repo2"].DefaultPolicy, ShouldResemble, []string{"read", "write"})
			So(repositories["repo2"].Policies[0].Actions, ShouldResemble, []string{"read", "write"})
			So(repositories["repo2"].Policies[0].Users, ShouldResemble, []string{"user1"})
		})

		Convey("Test that mutations to retrieved AccessControlConfig work with nil initial config", func() {
			...
			newAccessControlConfig.SetAdminPolicy(config.Policy{
				Actions: []string{"read"},
				Users:   []string{"admin"},
			})
			...
			retrievedConfig.SetAdminPolicy(config.Policy{
				Actions: []string{"read", "write"},
			...
			So(adminPolicy.Users, ShouldResemble, []string{"admin", "user"})
		})
	})

	Convey("Test AccessControlConfig reference consistency through UpdateReloadableConfig()", t, func() {
		Convey("Test that old AccessControlConfig reference retrieves new data after UpdateReloadableConfig", func() {
			...
			// Verify that the old reference now retrieves the new data
			updatedAdminPolicy := initialAccessControlConfig.GetAdminPolicy()
			So(updatedAdminPolicy.Actions, ShouldResemble, []string{"read", "write", "delete"})
			So(updatedAdminPolicy.Users, ShouldResemble, []string{"admin", "superadmin", "user"})

			updatedRepositories := initialAccessControlConfig.GetRepositories()
			So(len(updatedRepositories), ShouldEqual, 2)
			So(updatedRepositories["repo1"], ShouldNotBeNil)
			So(updatedRepositories["repo2"], ShouldNotBeNil)
			So(updatedRepositories["repo1"].DefaultPolicy, ShouldResemble, []string{"read", "write"})
			So(updatedRepositories["repo2"].DefaultPolicy, ShouldResemble, []string{"read"})
			So(updatedRepositories["repo2"].Policies[0].Actions, ShouldResemble, []string{"read"})
			So(updatedRepositories["repo2"].Policies[0].Users, ShouldResemble, []string{"user1", "user2"})

			// Verify that a new reference also gets the same updated data
			newAccessControlConfig := cfg.GetAccessControlConfig()
			So(newAccessControlConfig, ShouldNotBeNil)
			So(newAccessControlConfig, ShouldEqual, initialAccessControlConfig) // Same reference
	...
	Convey("Test ExtensionConfig mutation through GetExtensionsConfig()", t, func() {
		Convey("Test that mutations to retrieved ExtensionConfig are reflected in original config", func() {
			...
			// Mutate the retrieved ExtensionConfig
			extensionConfig.SetMetricsPrometheusPath("/custom/metrics")

			// Add a new sync config
			newSyncConfig := &syncconf.Config{
				Enable: &enabled,
				Registries: []syncconf.RegistryConfig{
					{
						URLs: []string{"http://registry1:5000"},
					},
				},
			}
			extensionConfig.SetSyncConfig(newSyncConfig)

			// Add a new CVE config
			newCVEConfig := &extconf.CVEConfig{
				UpdateInterval: time.Hour * 2,
				Trivy: &extconf.TrivyConfig{
					DBRepository: "custom/trivy-db",
				},
			}
			extensionConfig.SetSearchCVEConfig(newCVEConfig)

			// Add a new scrub config
			...
			extensionConfig.SetScrubConfig(newScrubConfig)

			// Verify that the changes are reflected in the original config
			updatedExtensionConfig := cfg.GetExtensionsConfig()
			So(updatedExtensionConfig, ShouldNotBeNil)

			// Check that metrics path was updated
			So(updatedExtensionConfig.Metrics.Prometheus.Path, ShouldEqual, "/custom/metrics")

			// Check that sync config was updated
			So(updatedExtensionConfig.Sync, ShouldEqual, newSyncConfig)
			So(updatedExtensionConfig.Sync.Enable, ShouldNotBeNil)
			So(*updatedExtensionConfig.Sync.Enable, ShouldBeTrue)
			So(len(updatedExtensionConfig.Sync.Registries), ShouldEqual, 1)
			So(updatedExtensionConfig.Sync.Registries[0].URLs[0], ShouldEqual, "http://registry1:5000")

			// Check that CVE config was updated
			So(updatedExtensionConfig.Search.CVE, ShouldEqual, newCVEConfig)
			So(updatedExtensionConfig.Search.CVE.UpdateInterval, ShouldEqual, time.Hour*2)
			So(updatedExtensionConfig.Search.CVE.Trivy.DBRepository, ShouldEqual, "custom/trivy-db")

			// Check that scrub config was updated
			So(updatedExtensionConfig.Scrub, ShouldEqual, newScrubConfig)
			So(updatedExtensionConfig.Scrub.Enable, ShouldNotBeNil)
			So(*updatedExtensionConfig.Scrub.Enable, ShouldBeTrue)
			So(updatedExtensionConfig.Scrub.Interval, ShouldEqual, time.Hour*24)
		})
		...
		Convey("Test that mutations to retrieved ExtensionConfig work with nil initial config", func() {
			...
			// Mutate the retrieved config
			retrievedConfig.SetMetricsPrometheusPath("/new/metrics")

			// Verify the changes are reflected
			finalConfig := cfg.GetExtensionsConfig()
			So(finalConfig.Metrics.Prometheus.Path, ShouldEqual, "/new/metrics")
		})
	})

	Convey("Test ExtensionConfig reference consistency through UpdateReloadableConfig()", t, func() {
		Convey("Test that old ExtensionConfig reference retrieves new data after UpdateReloadableConfig", func() {
			...
			// Verify that the old reference now retrieves the new data
			// Note: UpdateReloadableConfig only updates specific parts, not the entire Extensions
			// So Metrics.Prometheus.Path should remain unchanged
			So(initialExtensionConfig.Metrics.Prometheus.Path, ShouldEqual, "/metrics")

			// But Sync, CVE, and Scrub should be updated
			So(initialExtensionConfig.Sync, ShouldNotBeNil)
			So(initialExtensionConfig.Sync.Enable, ShouldNotBeNil)
			So(*initialExtensionConfig.Sync.Enable, ShouldBeTrue)
			So(len(initialExtensionConfig.Sync.Registries), ShouldEqual, 1)
			So(len(initialExtensionConfig.Sync.Registries[0].URLs), ShouldEqual, 2)
			So(initialExtensionConfig.Sync.Registries[0].URLs[0], ShouldEqual, "http://registry1:5000")
			So(initialExtensionConfig.Sync.Registries[0].URLs[1], ShouldEqual, "http://registry2:5000")

			So(initialExtensionConfig.Search.CVE, ShouldNotBeNil)
			So(initialExtensionConfig.Search.CVE.UpdateInterval, ShouldEqual, time.Hour*2)
			So(initialExtensionConfig.Search.CVE.Trivy.DBRepository, ShouldEqual, "updated/trivy-db")

			So(initialExtensionConfig.Scrub, ShouldNotBeNil)
			So(initialExtensionConfig.Scrub.Enable, ShouldNotBeNil)
			So(*initialExtensionConfig.Scrub.Enable, ShouldBeTrue)
			So(initialExtensionConfig.Scrub.Interval, ShouldEqual, time.Hour*12)

			// Verify that a new reference also gets the same updated data
			newExtensionConfig := cfg.GetExtensionsConfig()
			So(newExtensionConfig, ShouldNotBeNil)
			So(newExtensionConfig, ShouldEqual, initialExtensionConfig) // Same reference
