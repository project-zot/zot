...
// extractIdentityFromSource attempts to extract identity from a specific source.
func extractIdentityFromSource(cert *x509.Certificate, source string, mtlsConfig *config.MTLSConfig) (string, error) {
	...
	case "commonname", "cn":
		if cert.Subject.CommonName == "" {
			return "", fmt.Errorf("%w", zerr.ErrNoIdentityInCommonName)
		}
	...
	case "url", "uri":
		if len(cert.URIs) == 0 {
			return "", fmt.Errorf("%w", zerr.ErrNoURISANFound)
		}
		idx := 0
		if mtlsConfig != nil {
	...
	case "dnsname", "dns":
		if len(cert.DNSNames) == 0 {
			return "", fmt.Errorf("%w", zerr.ErrNoDNSANFound)
		}
		idx := 0
		if mtlsConfig != nil {
	...
	case "email", "rfc822name":
		if len(cert.EmailAddresses) == 0 {
			return "", fmt.Errorf("%w", zerr.ErrNoEmailSANFound)
		}
		idx := 0
		if mtlsConfig != nil {
	...
	default:
		return "", fmt.Errorf("%w: %s", zerr.ErrUnsupportedIdentitySource, source)
	}
}

// extractMTLSIdentity extracts identity from certificate using configured sources with fallback chain.
func extractMTLSIdentity(cert *x509.Certificate, mtlsConfig *config.MTLSConfig) (string, error) {
	sources := []string{"CommonName"} // Default
	if mtlsConfig != nil && len(mtlsConfig.IdentitySources) > 0 {
		sources = mtlsConfig.IdentitySources
	}

	var lastErr error

	for _, source := range sources {
		identity, err := extractIdentityFromSource(cert, source, mtlsConfig)
		if err == nil && identity != "" {
			return identity, nil
		}
		lastErr = err
	...
	return "", fmt.Errorf("no identity found in any configured source: %w", lastErr)
}
