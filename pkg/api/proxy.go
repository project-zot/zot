...
func proxyHTTPRequest(ctx context.Context, req *http.Request,
	...
	// Get HTTP config safely
	httpConfig := ctrlr.Config.GetHTTPConfig()
	...
	clientOpts := common.HTTPClientOptions{
		TLSEnabled: httpConfig.TLS != nil,
		VerifyTLS:  httpConfig.TLS != nil, // for now, always verify TLS when TLS mode is enabled
		Host:       targetMember,
	}
