...
func (rh *RouteHandler) SetupRoutes() {
	...
	// Get HTTP config safely for CORS
	httpConfig := rh.c.Config.GetHTTPConfig()
	applyCORSHeaders := getCORSHeadersHandler(httpConfig.AllowOrigin)
	...
	if authConfig.IsAPIKeyEnabled() {
		...
		apiKeyRouter.Use(zcommon.CORSHeadersMiddleware(httpConfig.AllowOrigin))
	...
	if authConfig.IsBasicAuthnEnabled() || authConfig.IsBearerAuthEnabled() {
		...
		if request.Header.Get(constants.SessionClientHeaderName) != constants.SessionClientHeaderValue {
			...
			if authConfig.Bearer != nil {
				response.Header().Set("WWW-Authenticate", "bearer realm="+authConfig.Bearer.Realm)
			} else {
				response.Header().Set("WWW-Authenticate", "basic realm="+httpConfig.Realm)
			}
		}
	}
...
func (rh *RouteHandler) UpdateManifest(response http.ResponseWriter, request *http.Request) {
	...
	if !storageCommon.IsSupportedMediaType(httpConfig.Compat, mediaType) {
