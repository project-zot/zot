...
func (rh *RouteHandler) GetBlob(response http.ResponseWriter, request *http.Request) {
	...
	if partial && len(byteRanges) > 1 {
		...
		for _, byteRange := range byteRanges {
			...
			if err != nil {
				rh.c.Log.Error().Err(err).Msg("failed fetching partial chunk for multipart")
			...
			if err == nil {
				...
				for {
					_, copyErr := io.CopyN(partWriter, repo, maxSize)
					if errors.Is(copyErr, io.EOF) {
						break
					} else if copyErr != nil {
						rh.c.Log.Error().Err(copyErr).Msg("failed to copy partial data into multipart response")
	...
	WriteDataFromReader(response, status, blen, responseMediaType, repo, rh.c.Log)
}
