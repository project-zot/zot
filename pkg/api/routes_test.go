...
func TestRoutes(t *testing.T) {
	Convey("Make a new controller", t, func() {
		...
		Convey("CheckBlob", func() {
			...
			So(header.Get("Content-Type"), ShouldEqual, "application/vnd.oci.image.layer.v1.tar+gzip")
			So(header.Get("Content-Length"), ShouldEqual, "1024")
			So(header.Get("Accept-Ranges"), ShouldEqual, "bytes")

			// Good Case - Multiple Accept (eStargz fallback)
			statusCode, header = testCheckBlob(
				map[string]string{"name": "GoodRepo", "digest": "1234"},
				map[string]string{
			...
			So(statusCode, ShouldEqual, http.StatusOK)
			So(header.Get("Content-Type"), ShouldEqual, "application/octet-stream") // Ensure fallback works!
		...
		Convey("GetBlob", func() {
			...
			statusCode, header, body = testGetBlob(
				map[string]string{"name": "GoodRepo", "digest": "sha256:dummy"},
				map[string]string{"Range": "bytes=0-5", "Accept": "application/octet-stream"},
				&mocks.MockedImageStore{
