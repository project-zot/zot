...
func TestAsyncOnDemand(t *testing.T) {
	...
	t.Run("typed-nil SyncOnDemand", func(t *testing.T) {
		t.Parallel()

		var onDemand *asyncOnDemandMock
...
func TestIsNilInterface(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{name: "non-nil pointer", value: &asyncOnDemandMock{}, want: false},
		{name: "struct value", value: plainSyncOnDemand{}, want: false},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			t.Parallel()

			if got := isNilInterface(testCase.value); got != testCase.want {
				t.Fatalf("isNilInterface() = %t, want %t", got, testCase.want)
			}
		})
	}
}
