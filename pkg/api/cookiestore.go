...
func NewCookieStore(
	...
	if uiConfig == nil || uiConfig.SessionDriver == nil {
		// If the UI config is nil or the session driver is not configured, then
		...
		switch uiConfig.SessionDriver["name"] {
		case storageConstants.RedisDriverName:
			{
				prefix, ok := rediscfg.GetString(uiConfig.SessionDriver, "keyprefix", false, log)
				if !ok {
					prefix = "zotsession"
				}
				...
				client, err := rediscfg.GetRedisClient(uiConfig.SessionDriver, log)
				if err != nil {
					return nil, err
				}
