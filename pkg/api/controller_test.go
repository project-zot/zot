...
func TestOpenIDMiddlewareWithRedisRemoteSessionStore(t *testing.T) {
	port := test.GetFreePort()
	baseURL := test.GetBaseURL(port)
	defaultVal := true
	...
	for _, testcase := range testCases {
		t.Run(testcase.testCaseName, func(t *testing.T) {
			...
			Convey("make controller", t, func() {
				...
				conf.Storage.RemoteSessionCache = true
				conf.Storage.SessionDriver = map[string]any{
					"name": "redis",
...
func TestPeriodicGC(t *testing.T) {
	...
	Convey("Periodic GC enabled for substore", t, func() {
		...
		So(string(data), ShouldContainSubstring,
			fmt.Sprintf("\"SubPaths\":{\"/a\":{\"RootDirectory\":\"%s\",\"Dedupe\":false,\"RemoteCache\":false,\"RemoteSessionCache\":false,\"GC\":true,\"Commit\":false,\"GCDelay\":1000000000,\"GCInterval\":86400000000000", subDir)) //nolint:lll // gofumpt conflicts with lll
	})
