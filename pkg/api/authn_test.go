...
func TestCookiestoreCleanup(t *testing.T) {
	...
	uiCfgTestCases := []struct {
		name string
		cfg  extconf.UIConfig
	}{
		{
			"empty UI config",
			extconf.UIConfig{},
	...
	for _, testCase := range uiCfgTestCases {
		Convey("Test cookiestore cleanup works with "+testCase.name, t, func() {
			...
			cookieStore, err := api.NewCookieStore(&testCase.cfg, storeController, log, nil, nil)
			So(err, ShouldBeNil)
	...
	Convey("Test cookiestore cleanup without permissions on rootDir", t, func() {
		...
		uiCfg := extconf.UIConfig{}

		cookieStore, err := api.NewCookieStore(&uiCfg, storeController, log, []byte("secret"), nil)
		So(err, ShouldBeNil)
...
func TestRedisCookieStore(t *testing.T) {
	...
	testCases := []struct {
		...
		inputUIExtConfig *extconf.UIConfig
	}{
		{
		...
		{
			"Cookie store creation should not fail if the UI Extension Config is nil",
			true,
			"",
			nil,
		},
	...
	for _, testCase := range testCases {
		Convey(testCase.testName, t, func() {
			cookieStore, err := api.NewCookieStore(testCase.inputUIExtConfig, storeController, log, nil, nil)
