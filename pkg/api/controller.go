...
func (c *Controller) Run() error {
	...
	httpConfig := c.Config.GetHTTPConfig()
	if httpConfig.Ratelimit != nil {
		if httpConfig.Ratelimit.Rate != nil {
			engine.Use(RateLimiter(c, *httpConfig.Ratelimit.Rate))
		}

		for _, mrlim := range httpConfig.Ratelimit.Methods {
	...
	addr := fmt.Sprintf("%s:%s", httpConfig.Address, httpConfig.Port)
	server := &http.Server{
		Addr:              addr,
		Handler:           c.Router,
	...
	if httpConfig.Port == "0" || httpConfig.Port == "" {
		chosenAddr, ok := listener.Addr().(*net.TCPAddr)
		if !ok {
			c.Log.Error().Str("port", httpConfig.Port).Msg("invalid addr type")
		...
		chosenPort, _ := strconv.ParseInt(httpConfig.Port, 10, 32)

		c.chosenPort = int(chosenPort)
	}

	if httpConfig.TLS != nil && httpConfig.TLS.Key != "" && httpConfig.TLS.Cert != "" {
		...
		if httpConfig.TLS.CACert != "" {
			clientAuth := tls.VerifyClientCertIfGiven
			if c.Config.IsMTLSAuthEnabled() {
				clientAuth = tls.RequireAndVerifyClientCert
			}

			caCert, err := os.ReadFile(httpConfig.TLS.CACert)
			if err != nil {
				c.Log.Error().Err(err).Str("caCert", httpConfig.TLS.CACert).Msg("failed to read file")
		...
		return server.ServeTLS(listener, httpConfig.TLS.Cert, httpConfig.TLS.Key)
	}
