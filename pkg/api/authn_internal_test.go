...
func TestAuthorizationSchemeHelpers(t *testing.T) {
	...
	request.Header.Set("Authorization", "bearer\ttoken")
	assert.True(t, hasBearerAuthorizationHeader(request), "expected bearer header to be detected with tab separator")

	request.Header.Set("Authorization", "Bearer \t token")
	assert.True(t, hasBearerAuthorizationHeader(request), "expected bearer header to be detected with mixed whitespace")
...
func TestGetUsernamePasswordBasicAuthAcceptsTabSeparator(t *testing.T) {
	t.Parallel()
...
func TestHashUUIDPreservesStableAPIKeyLookupDigest(t *testing.T) {
	t.Parallel()

	assert.Equal(t,
		"db8055e0e0307d5a016bec4dc338d69875eb0fb7e614a8b125b08fb082095d98",
		hashUUID("00000000-0000-4000-8000-000000000000"),
	)
}
...
func TestTryAuthnHandlersRejectsUnhandledAuthorizationBeforeAnonymous(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:   "bearer header without bearer auth",
			auth:   &config.AuthConfig{APIKey: true},
			header: "Bearer token",
		},
		{
			name:   "basic header without basic credential backend",
			auth:   &config.AuthConfig{},
			mtls:   true,
			header: "Basic " + base64.StdEncoding.EncodeToString([]byte("alice:password")),
		},
		{
			name:   "unknown authorization scheme",
			auth:   &config.AuthConfig{APIKey: true},
			header: "Digest token",
		},
		{
			name:      "empty basic header remains anonymous",
			auth:      &config.AuthConfig{},
			mtls:      true,
			header:    "Basic Og==",
			wantAllow: true,
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			(&AuthnMiddleware{htpasswd: NewHTPasswd(log.NewTestLogger()), log: log.NewTestLogger()}).
				tryAuthnHandlers(&Controller{Config: conf, Log: log.NewTestLogger()})(next).
				ServeHTTP(response, request)

			assert.Equal(t, test.wantAllow, called)
			if test.wantAllow {
				assert.Equal(t, http.StatusAccepted, response.Code)
			} else {
...
func TestTryAuthnHandlersBearerChallengeForV2DependsOnAnonymousPolicyShape(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "anonymous-only policies do not force bearer handshake",
			accessControl: &config.AccessControlConfig{
				Repositories: config.Repositories{
					"**": config.PolicyGroup{
						AnonymousPolicy: []string{constants.ReadPermission},
		...
		{
			name: "mixed anonymous and authenticated-only policies force bearer handshake",
			accessControl: &config.AccessControlConfig{
				Repositories: config.Repositories{
					...
					"private/**": config.PolicyGroup{
						Policies: []config.Policy{
							{Users: []string{"alice"}, Actions: []string{constants.ReadPermission}},
						},
					},
				},
			},
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			conf.HTTP.Auth = &config.AuthConfig{
				APIKey: true,
				Bearer: &config.BearerConfig{
					Realm:   "https://zot.example.test/token",
					Service: "zot",
				},
			}
			conf.HTTP.AccessControl = test.accessControl
			...
			if test.wantChallenge {
				assert.Equal(t,
					`Bearer realm="https://zot.example.test/token",service="zot",scope=""`,
					challenge,
				)
			} else {
...
func TestCheckVersionSupportChallengeHeaders(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "bearer challenge suppressed for anonymous-only policies",
			authConfig: &config.AuthConfig{
				Bearer: &config.BearerConfig{Realm: "https://auth.example.test/token", Service: "zot"},
				APIKey: true,
			},
		...
		{
			name: "bearer challenge advertised for mixed anonymous and authenticated policies",
			authConfig: &config.AuthConfig{
				Bearer: &config.BearerConfig{Realm: "https://auth.example.test/token", Service: "zot"},
				APIKey: true,
			},
