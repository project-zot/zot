...
func TestNormalizeTokenExchangeRequest(t *testing.T) {
	...
	t.Run("bearer authorization", func(t *testing.T) {
		...
		if !reflect.DeepEqual(tokenRequest.credentials, []tokenExchangeCredential{{Secret: "bearer-token"}}) {
			t.Fatalf("unexpected credentials: %#v", tokenRequest.credentials)
		}
	})
...
func TestWrappedCredentialBearerToken(t *testing.T) {
	...
	extractedToken, ok = bearerTokenFromAuthHeader("bearer\t" + token)
	if !ok || extractedToken != token {
		t.Fatalf("unexpected tab-separated bearer token: %q, %t", extractedToken, ok)
	}

	extractedToken, ok = bearerTokenFromAuthHeader("Bearer \t " + token)
	if !ok || extractedToken != token {
		t.Fatalf("unexpected mixed-whitespace bearer token: %q, %t", extractedToken, ok)
	}
...
func TestParseWrappedCredentialBearerTokenRejectsInvalidTokens(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if _, err := parseWrappedCredentialBearerToken(testCase.token); !errors.Is(err, zerr.ErrInvalidWrappedBearerCredential) {
...
func TestTokenExchangeCredentialIsAPIKey(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if got := testCase.credential.isAPIKey(); got != testCase.wantIsAPIKey {
				t.Fatalf("expected isAPIKey %t, got %t", testCase.wantIsAPIKey, got)
			}

			if got := testCase.credential.hasAPIKeySecret(); got != testCase.wantHasAPIKeySecret {
				t.Fatalf("expected hasAPIKeySecret %t, got %t", testCase.wantHasAPIKeySecret, got)
			}
		})
	}
}
