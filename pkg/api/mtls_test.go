...
func TestExtractMTLSIdentity(t *testing.T) {
	testCases := []mTLSTestCase{
		...
		},
		{
			...
			mtlsConfig: &config.MTLSConfig{
				IdentitySources: []string{"Subject"},
			},
		...
		},
		{
			...
			mtlsConfig: &config.MTLSConfig{
				IdentitySources: []string{"DNSName"},
			},
		...
		},
		{
			...
			mtlsConfig: &config.MTLSConfig{
				IdentitySources: []string{"Email", "DNSName", "CommonName"},
			},
			allowedUsers:     []string{"testuser"},
			expectedIdentity: "testuser",
			expectedStatus:   http.StatusNotFound,
		...
		},
		{
			...
			mtlsConfig: &config.MTLSConfig{
				IdentitySources: []string{"commonname"}, // lowercase
			},
			allowedUsers:     []string{"testuser"},
			expectedIdentity: "testuser",
			expectedStatus:   http.StatusNotFound,
		...
		},
		{
			...
			description:      "Fallback chain fails when no identity sources available",
		},
		{
		...
		},
		{
			...
			mtlsConfig: &config.MTLSConfig{
				IdentitySources: []string{"URL"}, // Try to extract from URL but cert has no URIs
			},
			allowedUsers:     []string{},
			expectedIdentity: "",
			expectedStatus:   http.StatusUnauthorized,
		...
		},
		{
			...
			mtlsConfig: &config.MTLSConfig{
				IdentitySources: []string{"InvalidSource"}, // Unsupported source
			},
			allowedUsers:     []string{},
			expectedIdentity: "",
			expectedStatus:   http.StatusUnauthorized,
