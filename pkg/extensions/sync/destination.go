...
func (registry *DestinationRegistry) CommitAll(repo string, imageReference ref.Ref) error {
	...
	for _, desc := range index.Manifests {
		...
		case ispec.MediaTypeImageManifest:
			if err := registry.copyManifest(repo, desc, manifestBlob, reference, tempImageStore, seen); err != nil {
		...
		case ispec.MediaTypeImageIndex:
			...
			for _, manifest := range indexManifest.Manifests {
				...
				if err := registry.copyManifest(repo, manifest, manifestBuf, manifest.Digest.String(),
...
func (registry *DestinationRegistry) copyManifest(repo string, desc ispec.Descriptor, manifestContent []byte,
