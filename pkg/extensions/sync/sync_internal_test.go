...
func TestService(t *testing.T) {
	Convey("trigger fetch tags error", t, func() {
		...
		service, err := New(conf, "", nil, t.TempDir(), storage.StoreController{}, &mocks.StreamManagerMock{}, mocks.MetaDBMock{}, log.NewTestLogger())
		So(err, ShouldBeNil)
	...
	Convey("test continue paths for specific error types in on-demand sync", t, func() {
		...
		service1, err := New(conf1, "", nil, t.TempDir(), storage.StoreController{}, &mocks.StreamManagerMock{}, mocks.MetaDBMock{}, log.NewTestLogger())
		So(err, ShouldBeNil)
		...
		service2, err := New(conf2, "", nil, t.TempDir(), storage.StoreController{}, &mocks.StreamManagerMock{}, mocks.MetaDBMock{}, log.NewTestLogger())
		So(err, ShouldBeNil)
