...
func (onDemand *BaseOnDemand) syncImage(ctx context.Context, repo, reference string,
	...
	// A pinned sync (the streaming background sync) must commit from the exact service that
	// served the streamed manifest, so restrict the attempt to that one service.
