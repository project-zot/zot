...
func NewRemoteRegistry(client *regclient.RegClient, hosts []string, logger log.Logger) Remote {
	registry := &RemoteRegistry{}

	registry.log = logger
	registry.hosts = hosts
	registry.client = client
	registry.host = hosts[0]
...
func (registry *RemoteRegistry) GetRepositories(ctx context.Context) ([]string, error) {
	...
	for _, host := range registry.hosts {
		repoList, err = registry.client.RepoList(ctx, host)
		if err != nil {
			registry.log.Error().Err(err).Str("remote", host).Msg("failed to list repositories in remote registry")
...
func (registry *RemoteRegistry) GetImageReference(repo, reference string) (ref.Ref, error) {
	...
	if ok {
		imageRefPath = fmt.Sprintf("%s/%s@%s", registry.host, repo, digest.String())
	} else {
		// is tag
		imageRefPath = fmt.Sprintf("%s/%s:%s", registry.host, repo, reference)
	}
...
func (registry *RemoteRegistry) GetTags(ctx context.Context, repo string) ([]string, error) {
	repoRefPath := fmt.Sprintf("%s/%s", registry.host, repo)
