...
func TestRepoListWithNewestImageWithScaleOutProxyLocalStorage(t *testing.T) {
	...
	Convey("In a local scale-out cluster, response should contain correct data for RepoListWithNewestImage", t, func() {
		for _, port := range ports {
			...
			// NOTE: this section has been commented out due to inconsistency
			// in the tags being returned. Needs further investigation.
			// since the functionality is part of core zot, it can be investigated
			// without scale out cluster mode.
			...
			// 	SoMsg(
			// 		fmt.Sprintf(
			// 			"expected repo tag %s:%s was not found", expectedRepoTag.repo, expectedRepoTag.tag,
			// 		),
			// 		found,
			// 		ShouldBeTrue,
			// 	)
