...
func TestRepoListWithNewestImageGql(t *testing.T) {
	Convey("Image information should not be returned if user is not authorized for repo", t, func() {
		...
		// admin user should be able to get repo info
		...
		So(responseStruct.Results[0].Name, ShouldEqual, "admin/zot-test")

		// non-admin user should not be able to get repo info
		...
		var nonAdminResponse zcommon.RepoWithNewestImageResponse
