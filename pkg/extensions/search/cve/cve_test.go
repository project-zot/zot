	...
	Convey("Unit test the CVE struct", t, func() {
		...
		scanner := mocks.CveScannerMock{
			ScanImageFn: func(ctx context.Context, image string) (cvemodel.ScanResult, error) {
				...
				if repo == repo7 {
					return cvemodel.ScanResult{CVEMap: map[string]zcommon.CVE{}}, ErrFailedScan
				}
		...
		cveInfo = cveinfo.BaseCveInfo{Log: log, Scanner: mocks.CveScannerMock{
			...
			ScanImageFn: func(ctx context.Context, image string) (cvemodel.ScanResult, error) {
				...
				return cvemodel.ScanResult{CVEMap: make(map[string]zcommon.CVE)}, nil
			},
		}, MetaDB: metaDB}
