...
func TestIsMultiarchImageScanable(t *testing.T) {
	Convey("multiarch", t, func() {
		...
		validManifestData := repodb.ManifestData{
			ManifestBlob: validManifestBlob,
			ConfigBlob:   validConfigBlob,
		}
		...
		err = repoDB.SetManifestData(digestValidManifest, validManifestData)
		So(err, ShouldBeNil)
		...
		err = repoDB.SetIndexData(godigest.FromBytes(indexBlob), repodb.IndexData{
			IndexBlob: indexBlob,
		})
		So(err, ShouldBeNil)

		err = repoDB.SetRepoTag("repo1", "valid", godigest.FromBytes(indexBlob), ispec.MediaTypeImageIndex)
		So(err, ShouldBeNil)
		...
		indexBlob, err = test.GetIndexBlobWithManifests([]godigest.Digest{
			digestValidManifest, digestManifestUnscannableLayer,
		})
		So(err, ShouldBeNil)
		...
		err = repoDB.SetRepoTag("repo1", "unscanable-layer", godigest.FromBytes(indexBlob), ispec.MediaTypeImageIndex)
		So(err, ShouldBeNil)
		...
		indexBlob, err = test.GetIndexBlobWithManifests([]godigest.Digest{
			digestValidManifest, digestUnmarshableManifest,
		})
		So(err, ShouldBeNil)
		...
		err = repoDB.SetRepoTag("repo1", "unmarshable", godigest.FromBytes(indexBlob), ispec.MediaTypeImageIndex)
		So(err, ShouldBeNil)

		// Manifest meta cannot be found
		digestMissingManifest := godigest.FromBytes([]byte("Some other string"))

		err = repoDB.SetRepoTag("repo1", "missing", digestMissingManifest, ispec.MediaTypeImageIndex)
		So(err, ShouldBeNil)

		// RepoMeta contains invalid digest
		err = repoDB.SetRepoTag("repo1", "invalid-digest", "invalid", ispec.MediaTypeImageIndex)
		So(err, ShouldBeNil)
		...
		Convey("Valid image should be scannable", func() {
			result, err := scanner.IsImageFormatScannable("repo1", "valid")
			So(err, ShouldBeNil)
			So(result, ShouldBeTrue)
		})

		Convey("Image with layers of unsupported types should be unscannable", func() {
			result, err := scanner.IsImageFormatScannable("repo1", "unscannable-layer")
			So(err, ShouldNotBeNil)
			So(result, ShouldBeFalse)
		})

		Convey("Image with unmarshable manifests should be unscannable", func() {
			result, err := scanner.IsImageFormatScannable("repo1", "unmarshable")
			So(err, ShouldNotBeNil)
			So(result, ShouldBeFalse)
		})

		Convey("Image with missing manifest meta should be unscannable", func() {
			result, err := scanner.IsImageFormatScannable("repo1", "missing")
			So(err, ShouldNotBeNil)
			So(result, ShouldBeFalse)
		})

		Convey("Image with invalid manifest digest should be unscannable", func() {
			result, err := scanner.IsImageFormatScannable("repo1", "invalid-digest")
			So(err, ShouldNotBeNil)
			So(result, ShouldBeFalse)
		})

		Convey("Image with unknown tag should be unscannable", func() {
			result, err := scanner.IsImageFormatScannable("repo1", "unknown-tag")
			So(err, ShouldNotBeNil)
			So(result, ShouldBeFalse)
		})

		Convey("Image with unknown repo should be unscannable", func() {
			result, err := scanner.IsImageFormatScannable("unknown-repo", "sometag")
			So(err, ShouldNotBeNil)
			So(result, ShouldBeFalse)
		})
