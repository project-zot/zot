...
func (digestInfo DigestInfo) GetRepoInfoByDigest(storeController storage.StoreController, repo string,
	digest string) ([]ImageInfoByDigest, error) {
	...
	for _, manifest := range manifests {
		...
		if ok {
			imageBlobManifest, err := digestInfo.LayoutUtils.GetImageBlobManifest(storeController, repo, imageDigest.String())
