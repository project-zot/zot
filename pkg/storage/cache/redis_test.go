...
// TestRedisBlobRefs covers PutBlobRef/GetBlobRefs/DeleteBlobRef, the ref-counting path
// imagestore's blobRefIndexer interface calls for a Redis-backed cache.
...
// TestRedisBlobRefsMocked covers DeleteBlobRef's redis-command error branches
// (SIsMember/SRem/HGet/SCard/HDel against the BlobRefs bucket) via redismock.
...
func TestRedisCacheError(t *testing.T) {
	...
	Convey("Redis unreachable", t, func() {
		...
		// PutBlobRef/DeleteBlobRef acquire their own redsync lock, separate from
		// PutBlob/DeleteBlob's above; exercise their lock-acquisition failures too.
		...
		for i, redisDriverParams := range tests {
			...
			Convey("DeleteBlob tests"+testID, func() {
				...
				Convey("DeleteBlob promotes a duplicate to origin when duplicates exist"+testID, func() {
					...
					// delete original val1: val2 is still a duplicate, so it gets promoted to origin
					mock.Regexp().ExpectSetNX(keyPrefix+"locks:key", `.*`, 8*time.Second).SetVal(true)
					mock.ExpectSIsMember(keyPrefix+constants.BlobsCache+":"+constants.DuplicatesBucket+":key",
						path.Join(pathPrefix, "val1")).SetVal(false)
					mock.ExpectHGet(keyPrefix+constants.BlobsCache+":"+constants.OriginalBucket, "key").
