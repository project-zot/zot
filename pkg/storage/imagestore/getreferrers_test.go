...
func TestGetReferrers(t *testing.T) {
	Convey("GetReferrers", t, func() {
		...
		Convey("a repo that was never created returns an empty index, not ErrRepoNotFound", func() {
			// Per the distribution spec, referrers on a repo with nothing pushed yet must
			// return 200 with an empty index, not ErrRepoNotFound (a 500 at the route
			// handler) - OCI conformance's "empty" test group asserts exactly this.
