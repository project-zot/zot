...
type MockedImageStore struct {
	...
	GetBlobUploadsFn       func(repo string) ([]string, error)
...
func (is MockedImageStore) GetBlobUploads(repo string) ([]string, error) {
	if is.GetBlobUploadsFn != nil {
		return is.GetBlobUploadsFn(repo)
	}
