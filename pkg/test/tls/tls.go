...
type CertificateOptions struct {
	...
	// For client certificates, this is required and will be set in the Subject.
	CommonName string
}
...
// GenerateIntermediateCACertToFile generates an intermediate CA certificate signed by the provided parent CA
// and writes generated key and cert to files.
// opts is optional and can be used to customize certificate settings, including CommonName.
func GenerateIntermediateCACertToFile(
	parentCACertPEM, parentCAKeyPEM []byte,
	certOutputPath, keyOutputPath string,
	opts ...*CertificateOptions,
) error {
	intermediateCACertBytes, intermediateCAKeyBytes, err := GenerateIntermediateCACert(parentCACertPEM,
		parentCAKeyPEM, opts...)
	if err != nil {
		return err
	}

	err = os.WriteFile(certOutputPath, intermediateCACertBytes, 0o600)
	if err != nil {
		return err
	}

	err = os.WriteFile(keyOutputPath, intermediateCAKeyBytes, 0o600)
	if err != nil {
		return err
	}
