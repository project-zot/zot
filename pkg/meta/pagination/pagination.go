...
type PageFinder interface {
	Add(detailedRepoMeta metaTypes.DetailedRepoMeta)
	Page() ([]metaTypes.RepoMetadata, common.PageInfo)
	Reset()
}
...
type RepoPageFinder struct {
	...
	pageBuffer []metaTypes.DetailedRepoMeta
}

func NewBaseRepoPageFinder(limit, offset int, sortBy metaTypes.SortCriteria) (*RepoPageFinder, error) {
	if sortBy == "" {
	...
	if _, found := metaTypes.SortFunctions()[sortBy]; !found {
	...
	return &RepoPageFinder{
		...
		offset:     offset,
		sortBy:     sortBy,
		pageBuffer: make([]metaTypes.DetailedRepoMeta, 0, limit),
	}, nil
}

func (bpt *RepoPageFinder) Reset() {
	bpt.pageBuffer = []metaTypes.DetailedRepoMeta{}
}

func (bpt *RepoPageFinder) Add(namedRepoMeta metaTypes.DetailedRepoMeta) {
	bpt.pageBuffer = append(bpt.pageBuffer, namedRepoMeta)
}

func (bpt *RepoPageFinder) Page() ([]metaTypes.RepoMetadata, common.PageInfo) {
	if len(bpt.pageBuffer) == 0 {
		return []metaTypes.RepoMetadata{}, common.PageInfo{}
	}

	pageInfo := &common.PageInfo{}

	sort.Slice(bpt.pageBuffer, metaTypes.SortFunctions()[bpt.sortBy](bpt.pageBuffer))
	...
	repos := make([]metaTypes.RepoMetadata, 0, len(detailedReposPage))
...
func NewBaseImagePageFinder(limit, offset int, sortBy metaTypes.SortCriteria) (*ImagePageFinder, error) {
	if sortBy == "" {
		sortBy = metaTypes.AlphabeticAsc
	}
...
func (bpt *ImagePageFinder) Add(namedRepoMeta metaTypes.DetailedRepoMeta) {
	bpt.pageBuffer = append(bpt.pageBuffer, namedRepoMeta)
}

func (bpt *ImagePageFinder) Page() ([]metaTypes.RepoMetadata, common.PageInfo) {
	if len(bpt.pageBuffer) == 0 {
		return []metaTypes.RepoMetadata{}, common.PageInfo{}
	}
	...
	repos := make([]metaTypes.RepoMetadata, 0)
	...
	partialTags := map[string]metaTypes.Descriptor{}
