...
func OnUpdateManifest(repo, reference, mediaType string, digest godigest.Digest, body []byte,
	...
	if isSignature {
		...
		if errGetLayers != nil {
			...
			err = metaDB.AddManifestSignature(repo, signedManifestDigest, metaTypes.SignatureMetadata{
...
func OnDeleteManifest(repo, reference, mediaType string, digest godigest.Digest, manifestBlob []byte,
	...
	if isSignature {
		err = metaDB.DeleteSignature(repo, signedManifestDigest, metaTypes.SignatureMetadata{
			SignatureDigest: digest.String(),
			SignatureType:   signatureType,
		})
		if err != nil {
