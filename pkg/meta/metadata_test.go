...
func TestMetadataConfigNegative(t *testing.T) {
	Convey("Cannot create User metadata - config disabled ", t, func() {
		...
		sctlr := api.NewController(srcConfig)
		enabled := false
		mstore, err := meta.FactoryUserMetadataStore(&msConfig.UserMetadataStoreConfig{
			RootDir: srcConfig.Storage.RootDirectory,
			Driver:  "local",
			Enabled: &enabled,
		}, sctlr.Log)
	...
	Convey("Cannot create config file - injected failure", t, func() {
		t.Helper()
		srcConfig := config.New()
		srcConfig.Storage.RootDirectory = t.TempDir()
		sctlr := api.NewController(srcConfig)
		...
		mstore, err := meta.NewUserMetadataLocalStore(&msConfig.UserMetadataStoreConfig{
			RootDir: srcConfig.Storage.RootDirectory,
			Driver:  "local",
			Enabled: &enabled,
		}, msConfig.UserMetadataLocalFile, sctlr.Log)

		if injected || true {
			So(err, ShouldNotBeNil)
			So(mstore, ShouldBeNil)
		}
	})
}
...
func TestUserMetadata(t *testing.T) {
	...
	Convey("UserMetadata", t, func(c C) {
		...
		conf.Log.Output = fmt.Sprintf("/var/log/zot/zot%s.log", startOfTest.Local().
			Format("20060201_150405"))
