...
`)
	...
	Convey("Test remote session config", t, func(c C) {
		tmpfile, err := os.CreateTemp("", "zot-test*.json")
		So(err, ShouldBeNil)
		...
		testCases := []struct {
			...
			{
				"Should fail verify if remote session storage is enabled, but no config is provided",
				[]byte(`{
					"storage":{
						"rootDirectory":"/tmp/zot"
					},
			...
			{
				"Should fail verify if remote session store is enabled, but invalid driver provided",
				[]byte(`{
					"storage":{
						"rootDirectory":"/tmp/zot"
					},
			...
			{
				"Should fail verify if remote session storage is enabled, but driver name is not provided",
				[]byte(`{
					"storage":{
						"rootDirectory":"/tmp/zot"
					},
