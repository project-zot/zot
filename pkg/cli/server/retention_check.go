...
func newRetentionCheckCmd(conf *config.Config) *cobra.Command {
	...
	retentionCheckCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !conf.Storage.GC {
				logger.Error().Msgf("failed to run retention-check, garbage collection is disabled in config")

				return fmt.Errorf("%w: %s", zerr.ErrBadConfig, "retention-check requires GC to be enabled")
			}

			// Set short delay for retention-check command
			...
			if conf.Storage.SubPaths != nil {
				for route, storageConfig := range conf.Storage.SubPaths {
					storageConfig.GCMaxSchedulerDelay = 500 * time.Millisecond
					if gcInterval > 0 {
