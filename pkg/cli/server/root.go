...
func validateRemoteSessionStoreConfig(cfg *config.Config, log zlog.Logger) error {
	if cfg.Storage.RemoteSessionCache {
		if cfg.Storage.SessionDriver == nil {
		...
		sessionDriverName, ok := cfg.Storage.SessionDriver["name"]
		if !ok {
...
func validateConfiguration(config *config.Config, log zlog.Logger) error {
	...
	if err := validateRemoteSessionStoreConfig(config, log); err != nil {
		return err
	}
