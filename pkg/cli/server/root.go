...
func validateRegistryStreamingSyncConfig(regCfg syncconf.RegistryConfig) error {
	...
	// onDemandInBackground answers a local miss immediately with a 404 while streaming answers it
	// by serving the upstream manifest right away - the two modes are contradictory.
	if regCfg.IsOnDemandInBackgroundEnabled() {
