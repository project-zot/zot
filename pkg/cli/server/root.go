...
func validateHTTP(config *config.Config, logger zlog.Logger) error {
	...
	if httpConfig.Port != "" {
		port, err := strconv.ParseInt(httpConfig.Port, 10, 64)
		if err != nil || (port < 0 || port > 65535) {
			logger.Error().Str("port", httpConfig.Port).Msg("invalid port")

			return fmt.Errorf("%w: invalid port %s", zerr.ErrBadConfig, httpConfig.Port)
		}
	}
