...
func TestRetentionCheckNegative(t *testing.T) {
	...
	Convey("Test retention-check no args", t, func(c C) {
		os.Args = []string{"cli_test", "retention-check"}
		err := cli.NewServerRootCmd().Execute()
		So(err, ShouldNotBeNil)
	})

	Convey("non-existent config", t, func(c C) {
		os.Args = []string{"cli_test", "retention-check", path.Join(os.TempDir(), "/x.yaml")}
		err := cli.NewServerRootCmd().Execute()
		So(err, ShouldNotBeNil)
	})

	Convey("unknown config", t, func(c C) {
		os.Args = []string{"cli_test", "retention-check", path.Join(os.TempDir(), "/x")}
		err := cli.NewServerRootCmd().Execute()
		So(err, ShouldNotBeNil)
	})
	...
	Convey("bad config", t, func(c C) {
		...
		os.Args = []string{"cli_test", "retention-check", "-t", "30s", configFile}
		err = cli.NewServerRootCmd().Execute()
		So(err, ShouldNotBeNil)
	})
	...
	Convey("config with GC disabled", t, func(c C) {
		...
		os.Args = []string{"cli_test", "retention-check", "-l", logFile, "-t", "30s", configFile}
		err = cli.NewServerRootCmd().Execute()

		// Verify the specific error
		So(err, ShouldNotBeNil)
		So(err.Error(), ShouldEqual,
			fmt.Sprintf("%s: %s", zerr.ErrBadConfig.Error(), "retention-check requires GC to be enabled"))
		...
		So(logStr, ShouldContainSubstring,
			"failed to run retention-check, garbage collection is disabled in config")
	})
	...
	Convey("server is running", t, func(c C) {
		...
		So(string(logContent), ShouldContainSubstring,
			"retention settings are enabled - the zot server must be stopped to access the metadata database")
		So(string(logContent), ShouldContainSubstring,
			"server is running, in order to perform the retention-check command the server should be shut down")
	})
}
...
func TestRetentionCheckWithRetentionEnabled(t *testing.T) {
	...
	Convey("valid config with retention enabled", t, func(c C) {
		...
		// Create complex image setup before running retention-check
		conf := config.New()
		...
		// Close metaDB to release database lock before running retention-check
		err = metaDB.Close()
		So(err, ShouldBeNil)

		time.Sleep(50 * time.Millisecond) // wait for GC delay to pass

		os.Args = []string{"cli_test", "retention-check", "-l", logFile, "-t", "1s", configFile}
		err = cli.NewServerRootCmd().Execute()
		So(err, ShouldBeNil)
		...
		// Verify basic retention-check and GC messages
		So(logStr, ShouldContainSubstring,
			"retention settings are enabled - the zot server must be stopped to access the metadata database")
		...
		expectedResults := []ExpectedRetentionResult{
			{
				Tag: "base-tag", Repository: "referrer-test-repo", Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "recent-tag-1", Repository: repo1, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "recent-tag-2", Repository: repo1, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "old-tag-1", Repository: repo1, Decision: "delete",
				Reason: "didn't meet any tag retention rule",
			},
			{
				Tag: "old-tag-2", Repository: repo1, Decision: "delete",
				Reason: "didn't meet any tag retention rule",
			},
			{
				Tag: "multiarch-tag", Repository: repo1, Decision: "delete",
				Reason: "didn't meet any tag retention rule",
			},
			...
			{
				Tag: "", Repository: repo1, Decision: "delete",
...
func TestRetentionCheckWithDeleteReferrers(t *testing.T) {
	...
	Convey("valid config with deleteReferrers enabled", t, func(c C) {
		...
		// Create image setup before running retention-check
		conf := config.New()
		err = cli.LoadConfiguration(conf, configFile)
		So(err, ShouldBeNil)
		...
		expectedResults := []ExpectedRetentionResult{
			// Tagged images
			{
				Tag: "recent-tag", Repository: repo, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "old-tag", Repository: repo, Decision: "delete",
				Reason: "didn't meet any tag retention rule",
			},
			// Untagged manifest deletions (old-tag image becomes untagged)
			{
				Tag: "", Repository: repo, Decision: "delete",
...
func TestRetentionCheckWithRetentionDisabled(t *testing.T) {
	...
	Convey("valid config with retention disabled", t, func(c C) {
		...
		// Verify basic retention-check messages
		So(logStr, ShouldContainSubstring,
		...
		expectedResults := []ExpectedRetentionResult{
			...
			{
				Tag: "", Repository: "gc-test-repo", Decision: "delete",
				Reason: "deleteUntagged", Digest: untaggedImage1.DigestStr(), IsUntagged: true,
			},

			// multiarch-test-repo: 4 untagged manifests deleted (multiarch index + 3 single-image manifests)
			{
				Tag: "", Repository: "multiarch-test-repo", Decision: "delete",
...
func TestRetentionCheckWithSubpaths(t *testing.T) {
	...
	Convey("config with subpaths", t, func(c C) {
		...
		expectedResults := []ExpectedRetentionResult{
			...
			{
				Tag: "recent-tag", Repository: repo1, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "multiarch-tag", Repository: repo1, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "old-tag", Repository: repo1, Decision: "delete",
				Reason: "didn't meet any tag retention rule",
			},
			...
			{
				Tag: "recent-tag", Repository: repo2, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "multiarch-tag", Repository: repo2, Decision: "keep",
				Reason: "retained by mostRecentlyPulledCount",
			},
			{
				Tag: "old-tag", Repository: repo2, Decision: "delete",
				Reason: "didn't meet any tag retention rule",
			},
			// Untagged manifest deletions in subpath
			{
				Tag: "", Repository: repo2, Decision: "delete",
...
func TestRetentionCheckWithGCIntervalOverride(t *testing.T) {
	...
	Convey("config with gc-interval override", t, func(c C) {
		...
		os.Args = []string{"cli_test", "retention-check", "-l", logFile, "-i", "30s", "-t", "5ms", configFile}
		err = cli.NewServerRootCmd().Execute()
		So(err, ShouldBeNil)
		...
		So(subpathConfig.GCMaxSchedulerDelay, ShouldEqual, 500000000) // 500ms (subpaths might have different default)
