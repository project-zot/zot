...
// BlobStore provides the image storage operations.
...
func (is *BlobStore) Name() string {
	return is.storeDriver.Name()
}

func (is *BlobStore) RootDir() string {
	return is.rootDir
}

func (is *BlobStore) DirExists(d string) bool {
	return is.storeDriver.DirExists(d)
}

// NewBlobStore returns a new image store backed by cloud storages.
// see https://github.com/docker/docker.github.io/tree/master/registry/storage-drivers
// Use the last argument to properly set a cache database, or it will default to boltDB local storage.
func NewBlobStore(rootDir string, cacheDir string, dedupe, commit bool, log zlog.Logger,
...
// RLock read-lock.
func (is *BlobStore) RLock(lockStart *time.Time) {
	*lockStart = time.Now()
...
// RUnlock read-unlock.
func (is *BlobStore) RUnlock(lockStart *time.Time) {
	is.lock.RUnlock()
...
// Lock write-lock.
func (is *BlobStore) Lock(lockStart *time.Time) {
	*lockStart = time.Now()
...
// Unlock write-unlock.
func (is *BlobStore) Unlock(lockStart *time.Time) {
	is.lock.Unlock()
...
func (is *BlobStore) initRepo(name string) error {
...
func (is *BlobStore) ValidateRepo(name string) (bool, error) {
	if !zreg.FullNameRegexp.MatchString(name) {
...
func (is *BlobStore) GetNextRepositories(lastRepo string, maxEntries int, filterFn storageTypes.FilterRepoFunc,
) ([]string, bool, error) {
	var lockLatency time.Time
...
func (is *BlobStore) GetRepositories() ([]string, error) {
	var lockLatency time.Time
...
func (is *BlobStore) GetNextRepository(repo string) (string, error) {
	var lockLatency time.Time
...
func (is *BlobStore) GetImageTags(repo string) ([]string, error) {
	var lockLatency time.Time
...
func (is *BlobStore) GetImageManifest(repo, reference string) ([]byte, godigest.Digest, string, error) {
	dir := path.Join(is.rootDir, repo)
	if fi, err := is.storeDriver.Stat(dir); err != nil || !fi.IsDir() {
		return nil, "", "", zerr.ErrRepoNotFound
	}
...
func (is *BlobStore) PutImageManifest(repo, reference, mediaType string, //nolint: gocyclo
	body []byte,
...
func (is *BlobStore) DeleteImageManifest(repo, reference string, detectCollisions bool) error {
	dir := path.Join(is.rootDir, repo)
	if fi, err := is.storeDriver.Stat(dir); err != nil || !fi.IsDir() {
...
func (is *BlobStore) BlobUploadPath(repo, uuid string) string {
	dir := path.Join(is.rootDir, repo)
...
*/
func (is *BlobStore) ListBlobUploads(repo string) ([]string, error) {
...
func (is *BlobStore) StatBlobUpload(repo, uuid string) (bool, int64, time.Time, error) {
...
func (is *BlobStore) NewBlobUpload(repo string) (string, error) {
	if err := is.InitRepo(repo); err != nil {
...
func (is *BlobStore) GetBlobUpload(repo, uuid string) (int64, error) {
	blobUploadPath := is.BlobUploadPath(repo, uuid)
...
func (is *BlobStore) PutBlobChunkStreamed(repo, uuid string, body io.Reader) (int64, error) {
	if err := is.InitRepo(repo); err != nil {
		return -1, err
	}
...
// the number of actual bytes to the blob.
func (is *BlobStore) PutBlobChunk(repo, uuid string, from, to int64,
	body io.Reader,
) (int64, error) {
	if err := is.InitRepo(repo); err != nil {
		return -1, err
...
func (is *BlobStore) BlobUploadInfo(repo, uuid string) (int64, error) {
	blobUploadPath := is.BlobUploadPath(repo, uuid)
...
func (is *BlobStore) FinishBlobUpload(repo, uuid string, body io.Reader, dstDigest godigest.Digest) error {
...
func (is *BlobStore) FullBlobUpload(repo string, body io.Reader, dstDigest godigest.Digest) (string, int64, error) {
	if err := dstDigest.Validate(); err != nil {
		return "", -1, err
	}
...
func (is *BlobStore) DedupeBlob(src string, dstDigest godigest.Digest, dstRepo string, dst string) error {
retry:
...
func (is *BlobStore) DeleteBlobUpload(repo, uuid string) error {
	blobUploadPath := is.BlobUploadPath(repo, uuid)
...
func (is *BlobStore) BlobPath(repo string, digest godigest.Digest) string {
	return path.Join(is.rootDir, repo, ispec.ImageBlobsDir, digest.Algorithm().String(), digest.Encoded())
}

func (is *BlobStore) GetAllDedupeReposCandidates(digest godigest.Digest) ([]string, error) {
	var lockLatency time.Time
...
*/
func (is *BlobStore) CheckBlob(repo string, digest godigest.Digest) (bool, int64, error) {
	var lockLatency time.Time
...
func (is *BlobStore) StatBlob(repo string, digest godigest.Digest) (bool, int64, time.Time, error) {
	if err := digest.Validate(); err != nil {
		return false, -1, time.Time{}, err
	}
...
func (is *BlobStore) checkCacheBlob(digest godigest.Digest) (string, error) {
	if err := digest.Validate(); err != nil {
		return "", err
	}
...
func (is *BlobStore) copyBlob(repo string, blobPath, dstRecord string) (int64, error) {
	if err := is.initRepo(repo); err != nil {
...
func (is *BlobStore) GetBlobPartial(repo string, digest godigest.Digest, mediaType string, from, to int64,
...
*/
func (is *BlobStore) originalBlobInfo(repo string, digest godigest.Digest) (driver.FileInfo, error) {
	blobPath := is.BlobPath(repo, digest)
...
// blob selector instead of directly downloading the blob.
func (is *BlobStore) GetBlob(repo string, digest godigest.Digest, mediaType string) (io.ReadCloser, int64, error) {
	var lockLatency time.Time
...
func (is *BlobStore) GetBlobContent(repo string, digest godigest.Digest) ([]byte, error) {
	if err := digest.Validate(); err != nil {
		return []byte{}, err
	}
...
func (is *BlobStore) VerifyBlobDigestValue(repo string, digest godigest.Digest) error {
	if err := digest.Validate(); err != nil {
		return err
	}
...
func (is *BlobStore) GetReferrers(repo string, gdigest godigest.Digest, artifactTypes []string,
) (ispec.Index, error) {
	var lockLatency time.Time
...
func (is *BlobStore) GetIndexContent(repo string) ([]byte, error) {
	dir := path.Join(is.rootDir, repo)
...
func (is *BlobStore) StatIndex(repo string) (bool, int64, time.Time, error) {
...
func (is *BlobStore) PutIndexContent(repo string, index ispec.Index) error {
	dir := path.Join(is.rootDir, repo)
...
func (is *BlobStore) DeleteBlob(repo string, digest godigest.Digest) error {
	var lockLatency time.Time
...
*/
func (is *BlobStore) CleanupRepo(repo string, blobs []godigest.Digest, removeRepo bool) (int, error) {
	count := 0
...
func getBlobDigest(imgStore *BlobStore, path string, digestAlgorithm godigest.Algorithm,
...
func (is *BlobStore) GetAllBlobs(repo string) ([]godigest.Digest, error) {
...
func (is *BlobStore) GetNextDigestWithBlobPaths(repos []string, lastDigests []godigest.Digest,
...
func (is *BlobStore) getOriginalBlobFromDisk(duplicateBlobs []string) (string, error) {
...
func (is *BlobStore) getOriginalBlob(digest godigest.Digest, duplicateBlobs []string) (string, error) {
...
func (is *BlobStore) dedupeBlobs(ctx context.Context, digest godigest.Digest, duplicateBlobs []string) error {
	if fmt.Sprintf("%v", is.cache) == fmt.Sprintf("%v", nil) {
...
func (is *BlobStore) restoreDedupedBlobs(ctx context.Context, digest godigest.Digest, duplicateBlobs []string) error {
...
func (is *BlobStore) RunDedupeForDigest(ctx context.Context, digest godigest.Digest, dedupe bool,
...
func (is *BlobStore) RunDedupeBlobs(interval time.Duration, sch *scheduler.Scheduler) {
...
func (is *BlobStore) PopulateStorageMetrics(interval time.Duration, sch *scheduler.Scheduler) {
