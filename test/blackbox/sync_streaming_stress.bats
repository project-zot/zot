...
function setup_file() {
    ...
    # readTimeout/writeTimeout default to 60s - far too short to write out a multi-GiB blob
    # (whether serving the sync client's fetch here, or the streamed response to test clients on
    # zot_stream below): the response gets cut off mid-stream at the deadline, and every reader of
    # that one shared stream ends up with the same truncated content (see the digest mismatch this
...
EOF
    ...
    # readTimeout/writeTimeout: see the comment on zot_minimal's config above - same reasoning,
    # this time for zot_stream's own streamed response to the test's clients below.
...
EOF
    ...
    # The upstream runs the full binary, not ZOT_MINIMAL_PATH: unlike sync_streaming.bats' small
    # fixture images (seeded via skopeo copy from a real registry), this test seeds its layer by
    # pushing directly to the upstream with oras, and the minimal build - built with no extensions
    # at all (see Makefile's binary-minimal target) - doesn't reliably support that here.
    ...
    # --insecure (skip TLS verify of zot_minimal's self-signed cert) is fine. oras refuses an
    # absolute file path by default (a safety check against leaking local paths into the pushed
    # artifact) - big_layer_file is absolute (under BATS_FILE_TMPDIR), so --disable-path-validation
...
@test "sync streaming: many concurrent clients pull a large not-yet-synced layer with matching content" {
    ...
    # The manifest GETs above must have gone through FetchManifestForStream (streaming), not
    # silently fallen back to plain on-demand sync (config typo'd "stream" out, maxConcurrentStreams
    # exceeded immediately, etc.) - confirm via its "syncing image in the background" log line,
    # which the plain on-demand path (runOnDemand's "starting on-demand image sync") never logs.
