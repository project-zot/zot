...
// audiencesExpr is the CEL expression for extracting the audiences from OIDC claims.
var audiencesExpr = func() *Expression {
...
func NewClaimProcessor(audiences []string, conf *config.CELClaimValidationAndMapping) (*ClaimProcessor, error) {
	// Validate audiences.
	if len(audiences) == 0 {
		return nil, fmt.Errorf("at least one audience must be specified")
	}
	for i, aud := range audiences {
		if aud == "" {
			return nil, fmt.Errorf("audience[%d] is empty", i)
		}
	}
	...
	for i, varConf := range conf.Variables {
		if varConf.Name == "" {
			return nil, fmt.Errorf("variable[%d] name is empty", i)
		}
	...
	for i, valConf := range conf.Validations {
		if valConf.Message == "" {
			return nil, fmt.Errorf("validation[%d] error message is empty", i)
		}
...
func (c *ClaimProcessor) Process(ctx context.Context, claims map[string]any) (*ClaimResult, error) {
	...
	for i := range c.validations {
		...
		if !val {
			return nil, fmt.Errorf("OIDC claim validation failed: %s", celVal.msg)
		}
	}
...
func (c *ClaimProcessor) validateAudience(ctx context.Context, claims map[string]any) error {
	audiences, err := audiencesExpr.EvaluateStringSlice(ctx, map[string]any{"claims": claims})
	if err != nil {
	...
	if !hasAudience {
		return fmt.Errorf("token audience %v does not match any of the expected audiences %v",
