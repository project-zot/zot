...
func TestNewClaimProcessor(t *testing.T) {
	for _, tt := range []struct {
		...
		},
		{
			...
			err: "validation[0] error message is empty",
		},
		{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			processor, err := cel.NewClaimProcessor(tt.audiences, tt.conf)

			if tt.err != "" {
				g.Expect(err).To(HaveOccurred())
				g.Expect(err.Error()).To(ContainSubstring(tt.err))
				g.Expect(processor).To(BeNil())
			} else {
...
func TestClaimProcessor_Process(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := processor.Process(context.Background(), tt.claims)
			...
			if tt.err != "" {
				...
				g.Expect(result.Username).To(Equal(tt.username))
				g.Expect(result.Groups).To(Equal(tt.groups))
			}
		})
	}
}
...
func TestClaimProcessor_Process_AudienceEdgeCases(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			processor, err := cel.NewClaimProcessor(tt.audiences, nil)
			g.Expect(err).NotTo(HaveOccurred())
